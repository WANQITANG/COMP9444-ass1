...
# with tf.Session() as sess:
#     print(sess.run(af,feed_dict={c3:5.0}))
...
def my_relu(in_value):
    ...
    c=tf.constant(0.0)
    out_value=tf.cond(in_value<=c,lambda :c,lambda :in_value)
    return out_value
...
def my_perceptron(x):
    ...
    Weights=tf.get_variable(name='Weights',shape=[1,x],initializer=tf.ones_initializer())
    j = tf.reshape(i, shape=[x,1])
    W_plus_b=tf.matmul(Weights,j)
    W_plus_b=tf.reshape(W_plus_b,shape=[])
    out=my_relu(W_plus_b)
...
def onelayer(X, Y, layersize=10):
    ...
    # dim_y=Y.shape()
    # print(dim_y)
    w=tf.Variable(tf.truncated_normal([dim,layersize],stddev=0.2))
    b=tf.Variable(tf.zeros([layersize])+0.05)
    # w=tf.get_variable(name='w',shape=[dim,layersize],initializer=tf.truncated_normal_initializer(stddev=0.2))
    # b=tf.get_variable(name='b',shape=[layersize],initializer=tf.constant_initializer(0.05))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    """
    dim = X.shape[1].value
    w1 = tf.Variable(tf.truncated_normal([dim,hiddensize],stddev=0.2))
    b1 = tf.Variable(tf.zeros([hiddensize])+0.05)
    w2 = tf.Variable(tf.truncated_normal([hiddensize, outputsize],stddev=0.2))
    b2 = tf.Variable(tf.zeros([outputsize])+0.05)
    layer1=tf.matmul(X,w1)+b1
    X_hat=tf.nn.relu(layer1)
    logits=tf.matmul(X_hat,w2)+b2
    preds=tf.nn.softmax(logits)
    batch_xentropy=tf.reduce_sum(-Y*tf.log(preds),reduction_indices=[1])
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    conv1=tf.layers.conv2d(X,filters=convlayer_sizes[0],kernel_size=filter_shape,activation=tf.nn.relu,padding='SAME')
    conv2=tf.layers.conv2d(conv1,filters=convlayer_sizes[1],kernel_size=filter_shape,activation=tf.nn.relu,padding='SAME')
    # print(conv2)
    output=tf.reshape(conv2,shape=[-1,784*convlayer_sizes[1]])
    w, b, logits, preds, batch_xentropy, batch_loss=onelayer(output,Y,outputsize)
    # dim=output.shape[1].value
    # w = tf.Variable(tf.truncated_normal(shape=[dim, outputsize],stddev=0.2))
    # b = tf.Variable(tf.zeros([outputsize])+0.05)
